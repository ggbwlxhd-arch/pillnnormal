...
import { Member, Goal, PainRecord, calcR, rc, pregW } from '@/lib/utils';
import EditMemberModal from '@/components/admin/EditMemberModal';

export default function MemberCard({
  member, longGoals, monthGoals, painRecs, onRefresh,
}: {
  member: Member;
  longGoals: Goal[];
  monthGoals: Goal[];
    ...
    <>
      <div style={{ background: '#fff', borderRadius: 20, boxShadow: '0 1px 3px rgba(0,0,0,.06)', overflow: 'hidden' }}>
        <div style={{ padding: '16px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            ...
            <div>
              ...
              {member.notes && (
                <p style={{ fontSize: 12, color: '#9CA3AF', marginTop: 3 }}>{member.notes}</p>
              )}
            </div>
          </div>
        ...
        <div style={{ padding: '0 20px 16px', display: 'flex', gap: 8 }}>
          <Link href={`/admin/members/${member.id}`} style={{ flex: 2, textAlign: 'center', padding: 11, background: '#3182F6', color: '#fff', borderRadius: 14, fontSize: 13, fontWeight: 700, textDecoration: 'none' }}>
            📋 일지 관리
          </Link>
          <button onClick={copyLink} style={{ flex: 2, padding: 11, background: '#F3F4F6', color: '#374151', borderRadius: 14, fontSize: 13, fontWeight: 700, border: 'none', cursor: 'pointer' }}>
            🔗 링크 복사
          </button>
          <button onClick={() => setShowEdit(true)} style={{ flex: 1, padding: 11, background: '#F9FAFB', color: '#374151', borderRadius: 14, fontSize: 13, fontWeight: 700, border: '1.5px solid #E5E7EB', cursor: 'pointer' }}>
            ✏️
          </button>
        </div>
      </div>

...
export default function MemberSharePage() {
  ...
  const [noticeDetail, setNoticeDetail] = useState<any | null>(null);
  ...
  async function fetchAll() {
    ...
    const [{ data: lg }, { data: mg }, { data: pr }, { data: di }, { data: cm }, { data: nts }] = await Promise.all([
      ...
      supabase.from('notices').select('*').eq('is_active', true).order('sort_order').order('created_at', { ascending: false }),
    ]);
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F3F4F6', fontFamily: "-apple-system,'Apple SD Gothic Neo',sans-serif" }}>
      ...
      <div style={{ background: '#fff', padding: '20px 20px 0', maxWidth: 480, margin: '0 auto' }}>
        {/* 3번: 공지 배너 */}
        {notices.length > 0 && (
          <div style={{ margin: '0 -20px', marginBottom: 0 }}>
            {notices.map(n => {
              const title = n.title.replace(/이름/g, member?.name?.replace('님', '') ?? '');
              return (
                <div key={n.id} onClick={() => setNoticeDetail(n)} style={{ background: n.image_url ? `url(${n.image_url}) center/cover` : n.bg_color, cursor: 'pointer', padding: '14px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <div style={{ background: n.image_url ? 'rgba(0,0,0,.35)' : 'transparent', padding: n.image_url ? '8px 12px' : 0, borderRadius: 8, flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <p style={{ fontSize: 14, fontWeight: 800, color: n.image_url ? '#fff' : n.text_color }}>{title}</p>
                    <span style={{ fontSize: 18, color: n.image_url ? '#fff' : n.text_color, opacity: .8 }}>›</span>
                  </div>
                </div>
              );
            })}
        ...
        {/* 공지 내용 팝업 */}
        {noticeDetail && (
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(17,24,39,.5)', zIndex: 200, display: 'flex', alignItems: 'flex-end', justifyContent: 'center', backdropFilter: 'blur(2px)' }} onClick={() => setNoticeDetail(null)}>
            <div onClick={e => e.stopPropagation()} style={{ background: '#fff', borderRadius: '24px 24px 0 0', width: '100%', maxWidth: 480, maxHeight: '80vh', overflowY: 'auto' }}>
              <div style={{ background: noticeDetail.bg_color, padding: '20px 24px', borderRadius: '24px 24px 0 0' }}>
                <p style={{ fontSize: 16, fontWeight: 800, color: noticeDetail.text_color }}>{noticeDetail.title.replace(/이름/g, member?.name?.replace('님', '') ?? '')}</p>
              </div>
              <div style={{ padding: '20px 24px' }}>
                <p style={{ fontSize: 14, color: '#374151', lineHeight: 1.8, whiteSpace: 'pre-wrap' }}>{noticeDetail.body?.replace(/이름/g, member?.name?.replace('님', '') ?? '')}</p>
                <button onClick={() => setNoticeDetail(null)} style={{ width: '100%', marginTop: 20, padding: 14, background: '#F3F4F6', color: '#374151', fontWeight: 700, fontSize: 14, borderRadius: 14, border: 'none', cursor: 'pointer' }}>닫기</button>
              </div>
            </div>
          </div>
        )}
